...
uses
  ...
  BinaryData, pngimage, jpeg, xPPM, xM8, xTGA, MMSystem, se_Duke3DUtils,
  se_Duke3DTypes, se_MyD3DUtils, se_DXInput, se_DirectX, se_DXClass, se_DXDraws,
  se_D3DUtils, se_Main, se_DXDUtils, se_Wave;

const
  vMax = DEFDUKE3DIMPORTFACTOR / 15.0; // Maximum navigation speed
  vMaxRotation : TD3DValue = 120.0; // Rotation / sec
  CULLDISTANCE = 10000.0;

type
  TDukeSoundRec = record
    SoundID: integer;
    SoundName: string;
...
type
  TDukeViewerScene = class(TD3DScene)
  public
  ...
  TDXViewerForm = class(TDXForm)
    ...
    procedure Rendersky1Click(Sender: TObject);
  ...
  private
    ...
    MapName: string;
    MainGRP, AdditionalGRP: TFileName;
    lBoost: integer;
    cFactor: integer;
    doThings: boolean;
    ...
    xShift: single; // 2D display offsets
    yShift: single; // 2D display offsets
    lastWidth,
    lastHeight: integer;
    isQueringPoint: boolean;
    qPoint: integer;
...
uses
  SyncObjs, Math, OpenDukeMapFrm, Clipbrd, Variants, QuickInfoFrm, PointInfoFrm,
  Splash, dk_io, dk_argv, dk_music, dk_globals, se_Utils;
...
{ *** TDukeViewerScene *** }
procedure TDukeViewerScene.UpdateObjectsTime;
var
  i: integer;
begin
...
procedure TDukeViewerScene.DrawLoop;
begin
  Inherited;
  if DXViewerForm.skyon and
    (DXViewerForm.Texture_sky2 <> nil) and
    (DXViewerForm.skyTexture <> '') then
  begin
...
begin
  ...
  AboutDialog1.ProductName := 'DukeViewer';
  AboutDialog1.Version := 'Version 0.9';
  AboutDialog1.Copyright := '© 2005-2018, Jim Valavanis, <jimmyvalavanis@yahoo.gr>';
  AboutDialog1.Comments := 'Duke Nukem 3D Map Viewer';

  AppConfigKey1 := TAppConfigKey.Create(self);
  AppConfigKey1.ApplicationName := 'DukeViewer';
  AppConfigKey1.ApplicationVersion := '0.9';
  ...
  regRenderSky := TVariantProfile.Create(self);
  regRenderSky.Key := regFormRestorer1;
  regRenderSky.Name := 'RenderSky';
  ...
  ErrorNoMapSelectedMessageBox.Caption := 'DukeViewer';
  ...
  ErrorNoFileSelectMessageBox.Caption := 'DukeViewer';
  ...
  ClipboardErrorMessageBox.Caption := 'DukeViewer';
  ...
  ErrorMessageBox.Caption := 'DukeViewer';
...
var
  i: integer;
begin
  if DK_CheckParam('-nosplash') = 0 then
  begin
  ...
  try
    ...
    Scene := TDukeViewerScene.Create(DXDraw);
    ...
    if not VarIsEmpty(regRenderSky.Value) then
      RenderSky1.Checked := regRenderSky.Value <> 0
    else
    begin
      RenderSky1.Checked := True;
      regRenderSky.Value := 1;
    end;
    AdjustSky;
    ...
    i := DK_CheckParam('-file');
    if i > 0 then
    begin
      MainGRP := DK_GetParam(i + 1);
      AdditionalGRP := MainGRP;
    end
    else
    begin
      MainGRP := '';
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (MainGRP <> '') and (AdditionalGRP <> '') then
      LoadMap;
...
begin
  ...
  if Rendersky1.Checked then
    regRenderSky.Value := 1
  else
    regRenderSky.Value := 0;
...
var
  ...
  var
    btm: TD3DValue;
  begin
    btm := Scene.BoundingCube.minY - DEFDUKE3DIMPORTFACTOR / 64 * 1.5;
    FloorInfo.y1 := btm;
    FloorInfo.y2 := btm;
    FloorInfo.y3 := btm;
    FloorInfo.y4 := btm;
...
begin
  ...
  try
    ...
    musname := DK_GetMusicName(MapName);
    songsize := 0;
    song := nil;
    if AdditionalGRP <> MainGRP then
      songGRP := AdditionalGRP
    else
      songGRP := MainGRP;
    if ReadDuke3DEntry(songGRP, musname, song, songsize) then
    begin
      if DK_RegisterSong(song, songsize) then
      begin
        DK_PlaySong;
        ResumeSong;
      end;
    ...
    GetDuke3DMapData(Scene, MainGRP, MapName, doThings, DEFDUKE3DIMPORTFACTOR, cFactor, lBoost);
    ...
    skyTexture := GetLastDuke3DSkyTexture;

    if skyTexture <> '' then
    begin
      Texture_sky2 := Scene.AddTextureToCollectionConst(skyTexture);
    ...
    begin
      minY1 := minY1 - DEFDUKE3DIMPORTFACTOR / 64 * 1.5;
      FloorInfo.y1 := minY1;
      FloorInfo.y2 := minY1;
      FloorInfo.y3 := minY1;
      FloorInfo.y4 := minY1;
...
begin
  ...
  try
    if QueryImportDukeMap(self, MainGRP, AdditionalGRP, MapName, cFactor, lBoost, doThings, s) then
    begin
...
procedure TDXViewerForm.Init2D;
begin
  fScale2D := 1.0;
  xShift := 0.0;
  yShift := 0.0;
  ...
  maxX := 0;
  maxY := 0;
  isQueringPoint := false;
  qPoint := -1;
end;

resourceString
  rsFmtTitle = 'DukeViewer - Map: %s - File: %s';

procedure TDXViewerForm.Read2DMap;
begin
  ClearMapData;
  ReadDuke3DMapData(AdditionalGRP, MapName,
    hSectors, numSectors,
    hWalls, numWalls,
    hSprites, numSprites);
  GetDuke3DMapDimentions(
    hWalls, numWalls,
    minX, minY, maxX, maxY);
  Caption := Format(rsFmtTitle, [MkShortName(MapName), MkShortName(AdditionalGRP)]);
end;
...
begin
  ...
  Preview2DDuke3DMap(
    ...
    hWalls, numWalls,
    hSprites, numSprites,
    Image1.Picture.Bitmap.Canvas, Image1.Width, Image1.Height, xShift, yShift, fScale2D,
    MinX, MinY, MaxX, MaxY, qPoint);
  Image1.Picture.Bitmap.Canvas.Font.Color := RGB(192, 192, 192);
  Image1.Picture.Bitmap.Canvas.Font.Style :=
    Image1.Picture.Bitmap.Canvas.Font.Style + [fsBold];
  Image1.Picture.Bitmap.Canvas.TextOut(5, 5, GetDuke3DDefMapName(MapName));
end;

procedure TDXViewerForm.ClearMapData;
begin
  if numSectors > 0 then
  begin
    ReallocMem(hSectors, 0);
    numSectors := 0;
  end;
...
procedure TDXViewerForm.AdjustSky;
begin
  skyon := RenderSky1.Checked;
end;
...
begin
  ...
  else if (Button = mbLeft) and ((ssCtrl in Shift) or (ssShift in Shift)) then
  begin
    p := GetDuke3D2DPreviewNearestWallIndex(
      hWalls, numWalls,
    ...
    if p >= 0 then
    begin
      if (hWalls[p].nextwall >= 0) then
      begin
        if (ssShift in Shift) then
          p := hWalls[p].nextwall
        else if (ssCtrl in Shift) and (p = qpoint) then
          p := hWalls[p].nextwall
      end;
    end;
    if p <> qPoint then
    ...
    if qPoint >= 0 then
      ShowDuke3DMapInfo(self, scene, qPoint, hSectors, numSectors, hWalls, numWalls);
  end;
  isQueringPoint := (ssCtrl in Shift) or (ssShift in Shift);
end;
...
procedure TDXViewerForm.Rendersky1Click(Sender: TObject);
begin
  Rendersky1.Checked := not Rendersky1.Checked;
  AdjustSky;
end;

procedure TDXViewerForm.Image1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
